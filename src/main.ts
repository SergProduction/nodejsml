import { Parse, LabeledDocs } from './parse'
...
const sliceDoc = (data: LabeledDocs, count: number) => (
  Object.fromEntries(
...
const main = () => {  
  const json = sliceDoc(jsonData, 40)
...
const printLabel = (label: string) => {
  const json = sliceDoc(jsonData, 5)
